...
async function fetchInsight(slug: string): Promise<DBInsight | null> {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL || ""}/api/insights?slug=${slug}`,
    {
      cache: "no-store",
      headers: {
  ...
  return (
    <>
      ...
      <div className="container py-4">
        <Header
          text={`Author: ${
            insight.author.includes("Mustafa Nadeem")
              ? `<a
                href="https://www.linkedin.com/in/mustafanadeemcfa"
                target="_blank"
                className="text-dark/50 hover:text-dark"
              >
                Mustafa Nadeem
              </a>`
              : `<span className="text-dark/50">${insight.author}</span>`
          } ${insight.co_author && `, Co-Author: <span className="text-dark/50">${insight.co_author}</span>`}`}
        />
        <div
          className="prose prose-md max-w-none mt-20"
  ...
  try {
    ...
    if (data) {
      ...
      seo = {
        ...
        og_title:
          data.seo?.og_title || data.seo?.title || data.title || seo.title,
        og_description:
        ...
        twitter_title:
          data.seo?.twitter_title || data.seo?.title || data.title || seo.title,
        twitter_description:
