...
function isMegaMenu(item: NavItemData | MegaMenuData): item is MegaMenuData {
  return (
    (item as MegaMenuData).isMegaMenu === true &&
    Array.isArray((item as MegaMenuData).children)
  );
}
...
export default function SiteSettings() {
  ...
  const [insightSlugs, setInsightSlugs] = useState<
    { label: string; slug: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const fetchAllInsights = async () => {
      ...
      let allInsights: any[] = [];
      while (true) {
        const res = await fetch(`/api/insights?page=${page}&pageSize=100`);
      ...
      setInsightSlugs(
        allInsights.map((i: any) => ({
          label: i.title,
          slug: `/insights/${i.slug}`,
        })),
      );
    };
  ...
  return (
    <section className="grid grid-cols-2 gap-4 container">
      ...
      <div className="mb-6">
        ...
        <div className="border p-4 border-dark/10 bg-surface mt-2">
          <div className="font-medium mb-2">
            Include/Exclude Pages from Sitemap
          </div>
          {excludeLoading ? (
            <div>Loading...</div>
          ) : (
            <>
              <div className="mb-2 text-xs text-gray-500">
                Uncheck to exclude a page from the sitemap.
              </div>
                ...
                {[...staticPages, ...insightSlugs].map((page) => (
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        handleExcludeChange(page.slug, !e.target.checked)
                      }
                    />
        ...
        <div className="flex gap-2 mt-4">
          <Button onClick={handleRegenerateSitemap}>Regenerate Sitemap</Button>
          <Button asChild variant="secondary">
      ...
      {message && (
        <div className="my-4 p-4 bg-surface col-span-2 border border-dark/10 text-dark capitalize">
          {message}
        </div>
      )}
    </section>
