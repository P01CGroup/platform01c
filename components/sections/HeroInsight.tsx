    ...
    >
      <div className="container min-h-[100dvh] md:min-h-[768px] pt-24 flex md:flex-row flex-col justify-between items-end pb-10">
        ...
        <div className="flex flex-col justify-end items-start">
          ...
          {title === "Feasibility Studies in Saudi Arabia" ? (
            <h2 className="heading-4 w-full max-w-[750px] leading-snug">
              Rigor That Speaks for Itself.
            </h2>
          ) : null}
          <p className="text-white/50 max-w-[600px]">{excerpt}</p>
        </div>
        <div className="flex items-end gap-1 flex-col">
          <span className="text-white/50">Published On</span>
          <span className="text-white">{formatDate(date, "long")}</span>
          {updatedDate && updatedDate !== date && (
            <>
              <span className="text-white/30 text-sm mt-2">Updated On</span>
